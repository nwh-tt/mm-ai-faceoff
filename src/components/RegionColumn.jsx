...
export default function RegionColumn({
  ...
  const r32matchups = Array.from({ length: 4 }, (_, i) => ({
    top: r32picks[i * 2]
      ? { seed: getSeed(r32picks[i * 2], teams), team: r32picks[i * 2] }
      : { seed: "?", team: "TBD" },
    bot: r32picks[i * 2 + 1]
      ? { seed: getSeed(r32picks[i * 2 + 1], teams), team: r32picks[i * 2 + 1] }
      : { seed: "?", team: "TBD" },
  ...
  const s16matchups = Array.from({ length: 2 }, (_, i) => ({
    top: s16picks[i * 2]
      ? { seed: getSeed(s16picks[i * 2], teams), team: s16picks[i * 2] }
      : { seed: "?", team: "TBD" },
    bot: s16picks[i * 2 + 1]
      ? { seed: getSeed(s16picks[i * 2 + 1], teams), team: s16picks[i * 2 + 1] }
      : { seed: "?", team: "TBD" },
  ...
  const e8matchup = {
    top: e8picks[0]
      ? { seed: getSeed(e8picks[0], teams), team: e8picks[0] }
      : { seed: "?", team: "TBD" },
    bot: e8picks[1]
      ? { seed: getSeed(e8picks[1], teams), team: e8picks[1] }
      : { seed: "?", team: "TBD" },
    pickedWinner: ffpick,
  ...
  return (
    <div className="flex flex-col gap-1">
      {/* Region header */}
      <div
        className={`flex items-center gap-2 mb-2 ${flip ? "flex-row-reverse" : ""}`}
      >
        <div
...
function RoundColumn({ label, slots, color, flip, teams }) {
  return (
    <div className="flex flex-col" style={{ width: 155 }}>
...
function SlotRow({ seed, team, status, color, isPicked }) {
  ...
  return (
    <div
      className="flex items-center gap-1.5 py-1.5 px-4 transition-colors"
      style={{
        background: bg,
        minHeight: 30,
        paddingLeft: 8,
    ...
    >
      <span
        ...
        style={{ fontSize: 12, color: color + "80", width: 16 }}
      >
        {seed}
      </span>
      <span
        className="font-condensed uppercase tracking-wide truncate flex-1"
        style={{
          fontSize: 13,
          color: textColor,
          fontWeight: 600,
